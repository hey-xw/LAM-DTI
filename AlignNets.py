...
class CTCModule(nn.Module):
    def __init__(self, in_dim, out_seq_len, args):
        ...
        self.pred_output_position_inclu_blank = nn.LSTM(in_dim, out_seq_len+1, num_layers=2, batch_first=True) # 1 denoting blank  加1是空白符
    ...
    def forward(self, x):
        ...
        pred_output_position_inclu_blank, _ = self.pred_output_position_inclu_blank(x) #模态到模态的对齐位置

        prob_pred_output_position_inclu_blank = self.softmax(pred_output_position_inclu_blank) # batch_size x in_seq_len x out_seq_len+1 概率分布
        #将概率分布中的空白符移除,得到了仅仅有预测符号的概率分布
...
class AlignSubNet(nn.Module):
    def __init__(self, args, mode):
        ...
        assert mode in ['avg_pool', 'ctc', 'sim']
        #首先维度要与conv之后的保持一致,
        in_dim_t, in_dim_v, in_dim_a = args.conv*4, args.conv*4, args.conv*4
        #序列长度怎么搞?不选择用conv对齐就可以不用
        ...
        if mode == 'ctc':
            ...
            self.shared_dim = args.shared_dim  #预先定义的,可以调参
    ...
    def __sim(self, text_x, video_x, audio_x):
        ...
        #text即ctx
        #其实只用的上计算ctx和药物,ctx和蛋白质的相似度进行对齐
        video_x = self.sim_v(video_x, text_x) if video_x.size(1) != self.dst_len else video_x  #在初始化的时候dst_len取的就是video_x的长度,所以不会执行!
